...
export default function App() {
  ...
  const stopCamera = useCallback(async () => {
    ...
    if (canvasRef.current) {
      ...
      await analyzeWithClaude(imageBase64, {}, "summary", 0, null);
    }
    if (streamRef.current) {
      streamRef.current.getTracks().forEach((t) => t.stop());
  ...
  return (
    <div className="app">
      <header>
        ...
        {cameraActive && <span className={`badge ${inFrame ? "detected" : poseDetected ? "searching" : "searching"}`}>
          {inFrame ? `✓ ${visibleCount}/33` : poseDetected ? `⚠ ${visibleCount}/33` : "Szukam…"}
        </span>}
      </header>
      ...
      <main>
        ...
        <div className="camera-wrap">
          ...
          {cameraActive && poseDetected && !inFrame && !loading && (
